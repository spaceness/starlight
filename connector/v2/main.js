import { app, BrowserWindow, Tray, Menu, dialog, ipcMain, shell } from 'electron'
...
ipcMain.handle('get-status', () => ({
    ...
    connectionUrl: isServerRunning ? `https://starlight-client.surge.sh/vnc.html?host=nostr.grooveix.com&path=${store.get('sessionId')}&port=443&encrypt=1&autoconnect=true` : null
}))
...
function updateTrayMenu(connectionUrl = null) {
    const contextMenu = Menu.buildFromTemplate([
        {
            label: `Connection ID: ${isServerRunning ? store.get('sessionId', 'Not Connected').split('-')[1]?.replace(/(\d{3})(?=\d)/g, '$1-') : 'Not Connected'}`,
            enabled: false
        },
        { type: 'separator' },
        {
